...
public class ParticipantsController implements Initializable {
  ...
  public void initialize(URL location, ResourceBundle resources) {
    File refreshFile = new File("resources/images/refresh.png");
    Image refreshIma = new Image(refreshFile.toURI().toString());
    refreshImage.setImage(refreshIma);
    ...
    try {
      ...
      clubCellFactory(clubBox);
      clubBox1.getItems().addAll(clubData);
      clubCellFactory(clubBox1);

      rankBox.setItems(rankData);
      rankCellFactory(rankBox);
      rankBox1.setItems(rankData);
      rankCellFactory(rankBox1);

      categoryBox.setItems(categoryData);
      categoryCellFactory(categoryBox);
      categoryBox1.setItems(categoryData);
      categoryCellFactory(categoryBox1);
      categoryRandom.setItems(categoryData);
      categoryCellFactory(categoryRandom);

      ageBox.setItems(ageData);
      ageCellFactory(ageBox);
      ageBox1.setItems(ageData);
      ageCellFactory(ageBox1);
      ageRandom.setItems(ageData);
      ageCellFactory(ageRandom);

      tourBox.setItems(tournamentData);
      tournamentCellFactory(tourBox);

      bindControls(participants);

    } catch (Exception e) {
      LOGGER.log(Level.SEVERE, "Unable to finish task!", e);
    }
    ...
    addParticipant.addEventHandler(MouseEvent.MOUSE_CLICKED, mouseEvent -> {
      if (Objects.equals(nameBox.getText(), "") || clubBox.getSelectionModel().getSelectedItem() != null || rankBox.getSelectionModel().getSelectedItem() != null || categoryBox.getSelectionModel().getSelectedItem() != null || ageBox.getSelectionModel().getSelectedItem() != null) {
        try {
          ...
          LOGGER.log(Level.SEVERE, "Couldn't add participant!", e);
    ...
    deleteParticipant.addEventHandler(MouseEvent.MOUSE_CLICKED, mouseEvent -> {
      if (participantsTable.getSelectionModel().getSelectedItem() != null) {
        ...
        try {
          ...
          LOGGER.log(Level.SEVERE, "Couldn't find participant!", e);
        }
      } else {
    ...
    updateParticipant.addEventHandler(MouseEvent.MOUSE_CLICKED, mouseEvent -> {
      if (!Objects.equals(nameBox.getText(), "") || clubBox.getSelectionModel().getSelectedItem() != null || rankBox.getSelectionModel().getSelectedItem() != null || categoryBox.getSelectionModel().getSelectedItem() != null || ageBox.getSelectionModel().getSelectedItem() != null) {
        try {
          ...
          LOGGER.log(Level.SEVERE, "Couldn't update!", e);
        }
      } else {
    ...
    listGenerator.addEventHandler(MouseEvent.MOUSE_CLICKED, mouseEvent -> {
      ...
      try {
        ...
        LOGGER.log(Level.SEVERE, "Couldn't generate list!", e);
      }
    });
    ...
    refreshBtn.setOnAction(event -> {
      if (participantsTable1.getSelectionModel().isEmpty()) {
        try {
  ...
  private void clubCellFactory(ComboBox<Club> clubBox) {
  ...
  private void rankCellFactory(ComboBox<Rank> rankBox) {
  ...
  private void categoryCellFactory(ComboBox<Category> categoryBox) {
  ...
  private void ageCellFactory(ComboBox<Age> ageBox) {
  ...
  private void tournamentCellFactory(ComboBox<Tournament> tournamentBox) {
    tournamentBox.setCellFactory(e -> new ListCell<Tournament>() {
      @Override
    ...
    tournamentBox.setConverter(new StringConverter<Tournament>() {
      @Override
